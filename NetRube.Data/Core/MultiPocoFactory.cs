...
namespace NetRube.Data.Internal
{
	class MultiPocoFactory
	{
					...
					for(int i = 1; i < types.Length; i++)
					{
						...
						for(int j = i - 1; j >= 0; j--)
						{
							...
							if(candidates.Count() > 1)
								throw new InvalidOperationException(Localization.Resource.CanNotAutoJoinAs.F(types[i], types[j]));
						...
						if(!handled)
							throw new InvalidOperationException(Localization.Resource.CanNotAutoJoin.F(types[i]));
					}
		...
		static Delegate FindSplitPoint(Type typeThis, Type typeNext, string ConnectionString, string sql, IDataReader r, ref int pos)
		{
			...
			throw new InvalidOperationException(Localization.Resource.CouldNotFindSplitPointException.F(typeThis, typeNext));
		}
